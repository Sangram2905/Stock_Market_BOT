...
if float(nf_n50['pChange']) >= 0.05 and float(nf_indiavix['pChange']) <0.0:
    print("Todays BUY CALLs from FnO Top Gainer Stock list",file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    for fg in range(len (df_fnogainers_join)):
        print("{} lotsize {} & above strike_price {}".format(df_fnogainers_join.index[fg],df_fnogainers_join.iloc[fg][0],df_fnogainers_join.iloc[fg][1]),file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    if len(df_lotsize_eqfnogainers_inx)>0:
        print("Todays BUY CALLs from Equity Top Gainer Stock list",file=open("NSEFnO_Todays TopGL.txt", "a"))
        print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
        for eg in range(len(df_lotsize_eqfnogainers_inx)):
            print("{} lotsize {} & above strike_price {}".format(df_eqfnogainers_join.index[eg],df_eqfnogainers_join.iloc[eg][0],df_eqfnogainers_join.iloc[eg][1]),file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("Todays BUY PUTs from FnO Top Loser Stock list",file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    for fl in range(len(df_fnolosers_join)):
        print("{} lotsize {} & below strike_price {}".format(df_fnolosers_join.index[fl],df_fnolosers_join.iloc[fl][0],df_fnolosers_join.iloc[fl][1]),file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))    
...
if float(nf_n50['pChange']) <= -0.05 and float(nf_indiavix['pChange']) >0.0:
    ...
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))    
    if len(df_lotsize_eqfnolosers_inx)>0:
        print("Todays BUY PUT from Equity Top Loser Stock list",file=open("NSEFnO_Todays TopGL.txt", "a"))
        print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
        for el in range(len(df_lotsize_eqfnolosers_inx)):
            print("{} lotsize {} & below strike_price {}".format(df_eqfnolosers_join.index[el],df_eqfnolosers_join.iloc[el][0],df_eqfnolosers_join.iloc[el][1]),file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("Todays BUY CALLs from FnO Top Gainer Stock list",file=open("NSEFnO_Todays TopGL.txt", "a"))
    print("",file=open("NSEFnO_Todays TopGL.txt", "a"))
    for fg in range(len (df_fnogainers_join)):
...
for df in range(len(df_lotmoney)):
    ...
    if (df_lotmoney.index[df] in df_fnolosers_join.index[0:]):
        print("PE {} lotsize {} & per 2000 price {}".format(df_lotmoney.index[df],df_lotmoney.iloc[df][0],df_lotmoney.iloc[df][2]))
        print('')
    if (df_lotmoney.index[df] in df_fnogainers_join.index[0:]):
        print("CE {} lotsize {} & per 2000 price {}".format(df_lotmoney.index[df],df_lotmoney.iloc[df][0],df_lotmoney.iloc[df][2]))
