...
for ind in range(len(dt_al)):
    if ((dt_al.iloc[ind][0:-1]>=dt_al.iloc[ind][-1]).all()):
        ...
        tx = "The Todays DayLow is less than previous 5 years avrage low "
        total_list.append([tx,dt_al.index[ind],dt_al.iloc[ind][-1]])
...
for ind in range(len(dt_ah)):
    if ((dt_ah.iloc[ind][0:-1]<=dt_ah.iloc[ind][-1]).all()):
        ...
        tx = "The Todays DayHigh is greter than previous 5 years avrage High "
        total_list.append([tx,dt_ah.index[ind],dt_ah.iloc[ind][-1]])
...
for ind in range(len(dt_cy)):
    if ((dt_cy.iloc[ind][1:-2:2]>=dt_cy.iloc[ind][-2]).all()):
        ...
        tx = "The Todays DayLow is less than current years avrage low "
        total_list.append([tx,dt_cy.index[ind],dt_cy.iloc[ind][-1]])
...
for ind in range(len(dt_cy)):
    if ((dt_cy.iloc[ind][0:-1:2]<=dt_cy.iloc[ind][-1]).all()):
        ...
        tx = "The Todays DayHigh is greter than current years avrage High "
        total_list.append([tx,dt_cy.index[ind],dt_cy.iloc[ind][-1]])
...
for ind in range(len(dt_shl_52)):
    if ((dt_shl_52.iloc[ind][1:-2:2]>=dt_shl_52.iloc[ind][-1]).all()):
        print("The Todays DayLow is less than current 52 week low "+dt_shl_52.index[ind],dt_shl_52.iloc[ind][-1])
        tx = "The Todays DayLow is less than current 52 week low "
        total_list.append([tx,dt_shl_52.index[ind],dt_shl_52.iloc[ind][-1]])

for ind in range(len(dt_shl_52)):
    if ((dt_shl_52.iloc[ind][0:-2:2]<=dt_shl_52.iloc[ind][-2]).all()):
        print("The Todays DayHigh is greter than current 52 week High "+dt_shl_52.index[ind],dt_shl_52.iloc[ind][-1])
        tx = "The Todays DayHigh is greter than current 52 week High "
        total_list.append([tx,dt_shl_52.index[ind],dt_shl_52.iloc[ind][-1]])
...
for ind in range(len(dt_fc)):
    if ((dt_fc.iloc[ind][1:-2:2]>=dt_fc.iloc[ind][-1]).all()):
        print("The Todays DayLow is less than financial crisis year low "+dt_fc.index[ind],dt_fc.iloc[ind][-1])
        tx = "The Todays DayLow is less than financial crisis year low "
        total_list.append([tx,dt_fc.index[ind],dt_fc.iloc[ind][-1]])


for ind in range(len(dt_fc)):
    if ((dt_fc.iloc[ind][0:-2:2]<=dt_fc.iloc[ind][-2]).all()):
        print("The Todays DayHigh is greter than financial crisis year High "+dt_fc.index[ind],dt_fc.iloc[ind][-1])
        tx = "The Todays DayLow is less than financial crisis year low "
        total_list.append([tx,dt_fc.index[ind],dt_fc.iloc[ind][-1]])
...
for ind in range(len(df_top_ln)):
    if ((df_top_ln.iloc[ind][3]>=df_top_ln.iloc[ind][0]).all()):
        ...
        tx = "The Todays top_losers DayLow is less than 52 week low "
        total_list.append([tx,df_top_ln.index[ind],df_top_ln.iloc[ind][0]])
...
for ind in range(len(df_top_gn)):
    if ((df_top_gn.iloc[ind][2]<=df_top_gn.iloc[ind][1]).all()):
        ...
        tx = "The Todays top_gainers DayHigh is greter than 52 week High "
        total_list.append([tx,df_top_gn.index[ind],df_top_gn.iloc[ind][1]])


totallist_df = pd.DataFrame(data=total_list).set_index(1)
